#!/usr/bin/env python3
"""
6-model ensemble inference for Tox21 multi-task toxicity prediction.

Members:
  1. ChemBERTa-2          (SMILES Transformer, 77M params)
  2. MoLFormer-XL         (SMILES Transformer, 44M params)
  3. AttentiveFP          (Graph, gated attention, from scratch)
  4. ECFP4 + XGBoost      (fingerprint, per-task XGBClassifier)
  5. GPS Graph Transformer (local GINEConv + global MultiheadAttention, from scratch)
  6. Pretrained GIN       (Hu et al. 2020, masking, 2M-molecule pretraining)  ← NEW

Also exhaustively searches all 2–6 model subsets.

Usage:
    conda activate drug-tox-env
    cd /media/nghia/HDD/PROJECT/Drug/molecule
    python scripts/ensemble6_tox21.py --device cuda
"""
...
ENSEMBLE_MEMBERS = [
    ...
    },
    {
        "name": "Pretrained-GIN",
        "type": "pretrained_gin",
        "model_dir": "models/tox21_pretrained_gin_model",
        "emb_dim": 300,
        "num_layers": 5,
        "drop_ratio": 0.0,   # no dropout at inference
        "jk": "last",
        "head_dropout": 0.0,
    },
]
...
def infer_smiles_model(member, smiles_list, labels, task_config, device, batch_size=32):
    ...
    model = create_pretrained_mol_model(
        pretrained_model=ckpt, num_tasks=task_config.num_tasks, dropout=0.0
    ).to(device)
    ...
    logits = torch.cat([
        model(b["input_ids"].to(device), b["attention_mask"].to(device)).cpu()
        for b in loader
    ]).numpy()
...
def infer_attentivefp(member, smiles_list, labels, task_config, device, batch_size=64):
    ...
    for batch in loader:
        ...
        bs    = out.shape[0]
        all_logits[ptr:ptr + bs] = out.cpu().numpy()
        ptr += bs
...
@torch.no_grad()
def infer_pretrained_gin(member, smiles_list, labels, task_config, device, batch_size=128):
    """Hu et al. pretrained GIN inference → probs (N, T)."""
    model_dir = project_root / member["model_dir"]

    # Build model (no pretrained download needed — load fine-tuned weights directly)
    from src.pretrained_gnn import HuGNNBackbone, GNNPretrainedPredictor
    backbone = HuGNNBackbone(
        emb_dim    = member["emb_dim"],
        num_layers = member["num_layers"],
        drop_ratio = member["drop_ratio"],
        jk         = member["jk"],
    )
    model = GNNPretrainedPredictor(
        backbone     = backbone,
        num_tasks    = task_config.num_tasks,
        head_dropout = member["head_dropout"],
    ).to(device)
    model.load_state_dict(torch.load(model_dir / "best_model.pt", map_location=device))
    model.eval()

    # Hu et al. featurization — track original indices to handle skipped molecules
    from src.pretrained_gnn import mol_to_graph_hu2020
    dataset, valid_idxs = [], []
    ...
    # Default logit=0 → sigmoid=0.5 for any skipped molecule
    all_logits = np.zeros((len(smiles_list), task_config.num_tasks), dtype=np.float32)
    ptr = 0
    for batch in loader:
        batch = batch.to(device)
        out   = model(batch.x, batch.edge_index, batch.edge_attr, batch.batch)
        bs    = out.shape[0]
        for k in range(bs):
            all_logits[valid_idxs[ptr + k]] = out[k].cpu().numpy()
        ptr += bs
    return 1.0 / (1.0 + np.exp(-all_logits))


def infer_xgb(member, smiles_list, labels, task_config, **_):
    model_dir = project_root / member["model_dir"]
    fps = []
    for smi in smiles_list:
        mol = Chem.MolFromSmiles(smi)
        if mol is None:
            fps.append(np.zeros(member["nbits"], dtype=np.float32))
        else:
            fp = rdMolDescriptors.GetMorganFingerprintAsBitVect(
                mol, member["radius"], nBits=member["nbits"])
...
def infer_member(member, smiles_list, labels, task_config, device, batch_size):
    t = member["type"]
    if   t == "smiles":         return infer_smiles_model(member, smiles_list, labels, task_config, device, batch_size)
    elif t == "graph_afp":      return infer_attentivefp (member, smiles_list, labels, task_config, device, batch_size)
    elif t == "graph_gps":      return infer_gps         (member, smiles_list, labels, task_config, device, batch_size)
    elif t == "xgb":            return infer_xgb         (member, smiles_list, labels, task_config)
    elif t == "pretrained_gin": return infer_pretrained_gin(member, smiles_list, labels, task_config, device, batch_size)
...
def compute_metrics(probs, labels, task_config):
    ...
    for t, name in enumerate(task_config.task_names):
        valid = ~np.isnan(labels[:, t]) & ~np.isnan(probs[:, t])
...
def save_comparison_figure(all_metrics, out_path, prev_best=0.7661):
    names  = list(all_metrics.keys())
    aucs   = [all_metrics[n]["mean_auc_roc"] for n in names]
    colors = ["#4C72B0", "#DD8452", "#55A868", "#C44E52", "#8172B2",
              "#937860", "#DA8BC3", "#8C8C8C", "#CCB974", "#64B5CD"]

    fig, ax = plt.subplots(figsize=(max(11, len(names) * 1.1), 4.5))
    ...
    ax.set_title("Tox21 — 6-Model Ensemble Analysis", fontsize=12)
    ax.axhline(prev_best, color="navy", linestyle="--", linewidth=0.9, alpha=0.7,
               label=f"5-model ensemble ({prev_best:.4f})")
    ax.axhline(0.7792, color="darkgreen", linestyle=":", linewidth=0.9, alpha=0.7,
               label="Pretrained-GIN single (0.7792)")
...
def save_pertask_heatmap(all_metrics, task_config, out_path):
    ...
    im = ax.imshow(data, vmin=0.62, vmax=0.88, cmap="RdYlGn", aspect="auto")
        ...
        for j in range(len(tasks)):
            ...
            if not np.isnan(val):
                ax.text(j, i, f"{val:.3f}", ha="center", va="center", fontsize=7,
                        color="black" if 0.67 < val < 0.84 else "white")
    plt.colorbar(im, ax=ax, label="AUC-ROC")
    ax.set_title("Per-Task AUC-ROC — 6-Model Ensemble Analysis", fontsize=12)
    plt.tight_layout()
    plt.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    print("=" * 70)
    print("Tox21 6-Model Ensemble")
    print("  1. ChemBERTa-2      2. MoLFormer-XL    3. AttentiveFP")
    print("  4. XGBoost          5. GPS              6. Pretrained-GIN")
    print("=" * 70)
    ...
    for member in ENSEMBLE_MEMBERS:
        print(f"\n[{member['name']}]")
        probs = infer_member(member, smiles_list, labels_array,
                             task_config, device, args.batch_size)
    ...
    # Combination search
    print("\n" + "=" * 70)
    print("Ensemble combination search (all 2–6 model subsets)")
    print("=" * 70)
    ...
    for size in range(2, len(names) + 1):
        for combo in combinations(names, size):
            avg   = np.mean([all_probs[n] for n in combo], axis=0)
            auc   = compute_metrics(avg, labels_array, task_config)["mean_auc_roc"]
            flag  = ""
            if auc > best_auc:
                best_auc   = auc
                best_combo = combo
                flag       = " ← best"
            # Print all combos involving Pretrained-GIN, and all that beat 0.77
            if "Pretrained-GIN" in combo or auc > 0.770 or size <= 2:
                print(f"  [{size}-model] {' + '.join(combo)}: AUC={auc:.4f}{flag}")

    print(f"\nBest ensemble: {' + '.join(best_combo)}  AUC={best_auc:.4f}")

    # Full 6-model ensemble
    print("\n[6-model ensemble — all]")
    full_probs   = np.mean(list(all_probs.values()), axis=0)
    full_metrics = compute_metrics(full_probs, labels_array, task_config)
    print_metrics(full_metrics, task_config, "6-Model Ensemble (all)")
    ...
    if set(best_combo) != set(names):
        print_metrics(best_metrics, task_config, f"Best combo ({' + '.join(best_combo)})")

    # Save
    out_dir = project_root / "models" / "tox21_ensemble6"
    ensure_dir(str(out_dir))

    flat = {
        "ensemble6_mean_auc_roc": full_metrics["mean_auc_roc"],
        "ensemble6_mean_pr_auc":  full_metrics["mean_pr_auc"],
        "best_combo":             " + ".join(best_combo),
        "best_combo_auc":         best_auc,
    }
    flat.update({f"ensemble6_auc_{k}": v for k, v in full_metrics["per_task_auc_roc"].items()})
    if set(best_combo) != set(names):
        flat.update({f"best_auc_{k}": v for k, v in best_metrics["per_task_auc_roc"].items()})
    save_metrics(flat, str(out_dir / "tox21_ensemble6_metrics.txt"))

    # Plot: individuals + key ensembles
    plot_metrics = {**all_metrics}
    for size in (3, 5, 6):
        key_combos = {
            3: ("ChemBERTa-2", "MoLFormer-XL", "AttentiveFP"),
            5: tuple(m["name"] for m in ENSEMBLE_MEMBERS[:5]),
            6: tuple(names),
        }
        combo = key_combos[size]
        label = f"Ens-{size}\n({'CB2+MF+AFP' if size==3 else 'prev' if size==5 else 'all-6'})"
        plot_metrics[label] = compute_metrics(
            np.mean([all_probs[n] for n in combo], axis=0), labels_array, task_config
        )
    if frozenset(best_combo) not in ({frozenset(key_combos[s]) for s in (3, 5, 6)}):
        plot_metrics[f"Best\n({len(best_combo)}-mdl)"] = best_metrics

    save_comparison_figure(plot_metrics, out_dir / "ensemble6_comparison.png")
    save_pertask_heatmap(plot_metrics, task_config, out_dir / "ensemble6_pertask_heatmap.png")
